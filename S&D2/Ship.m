...
- (Ship*)newShip:(NSArray*)params {
  ...
  if(self) {
    ...
    if ([params count] > PARAMS_PROPERTIES) {
      ...
      NSDictionary* partsFromCard = [properties objectForKey:@"aesthetics"];
      if(partsFromCard) {
        NSMutableDictionary* parts = [NSMutableDictionary new];
        for(NSString* bodyPart in [partsFromCard allKeys]) {
          [parts setObject:[partsFromCard valueForKey:bodyPart] forKey:[NSNumber numberWithInt:[BodyPartHelper TagForBodyPart:bodyPart]]];
        }
